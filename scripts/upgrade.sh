...
URL="https://codeload.github.com/000cuenta0001/mycacharrito/zip/master" 
...
echo El fichero local pesa: $SIZE_2 >> /storage/scripts/upgrade.log
 ...
 if [ $SIZE -ne $SIZE_2 ]; then
  ...
  unzip -o ${FILE_DIR}/pelis.zip mycacharrito-master/python/main-classic/* -d ${FILE_DIR} > /dev/null

  echo copia el plugin >> /storage/scripts/upgrade.log
  cp -rf ${FILE_DIR}/mycacharrito-master/python/main-classic/* /storage/.kodi/ > /dev/null
  ...
  rm -rf ${FILE_DIR}/mycacharrito-master
 fi
